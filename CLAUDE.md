...
5. **`reportCount`** — found 2026-08-09, logged not yet fixed. The daily-generation prompt asks the model for "a conservative estimate... if sources give no hard figure" and the result renders as a bare `"50,000 reports"`-style fact on both this site and the iOS app, where it also drives a live "Most Reported" sort. Cross-repo fix, not done tonight — see `content/REVIEW-QUEUE.md` for the full scope.
